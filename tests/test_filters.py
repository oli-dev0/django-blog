...
class BlogFilterViewTests(TestCase):
    ...
    def test_author_archive_initializes_author_filter(self):
        category = BlogCategory.objects.create(name='Django', slug='django')
        user = get_user_model().objects.create_user(username='example-author')
        author = AuthorProfile.objects.create(
            user=user,
        ...
        self.assertContains(response, 'Author: Example Author')
        self.assertContains(
            response,
            '<input type="radio" name="author" value="example-author" checked data-blog-filter-single>',
            html=True,
        )
        self.assertContains(
            response,
            'href="/en/blog/?type=guide&amp;author=example-author"',
            html=False,
        )
        self.assertNotContains(response, 'noindex,follow')
