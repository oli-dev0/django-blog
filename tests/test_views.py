...
from .site_settings import BLOG_PRESENTATION_SETTINGS, BLOG_ENABLED_SITE_DEFINITIONS
...
@override_settings(SITE_DEFINITIONS=BLOG_ENABLED_SITE_DEFINITIONS, **BLOG_PRESENTATION_SETTINGS)
class BlogViewTests(TestCase):
    ...
    def test_blog_list_renders_empty_state(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'personal_site/blog/list.html')
        self.assertContains(response, 'No articles have been published yet.')
        self.assertEqual(len(parser.links), 1)
        self.assertEqual(response.headers['Content-Language'], 'en')

    def test_product_blog_title_links_to_blog_homepage(self):
        response = self.client.get('/en/blog/', HTTP_HOST='product.localhost')

        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response,
            '<h1 id="blog-title"><a href="/en/blog/" data-blog-return-link>Product Site blog</a></h1>',
            html=True,
        )

    def test_product_blog_loads_only_required_shell_styles(self):
        self.create_post(site_slug=PRODUCT_SITE)

        for path in ('/en/blog/', '/en/blog/english-post/'):
            with self.subTest(path=path):
                response = self.client.get(path, HTTP_HOST='product.localhost')

                self.assertNotContains(response, static('product_site/css/styles.css'))
                for stylesheet in (
                    'core/css/fonts.css',
                    'product_site/css/base.css',
                    'product_site/css/layout.css',
                    'product_site/css/footer-newsletter.css',
                    'product_site/css/responsive-shared.css',
                    'product_site/css/responsive-final.css',
                    'product_site/css/blog.css',
                ):
                    self.assertContains(response, static(stylesheet))

    def test_product_blog_prioritizes_only_first_list_image_and_uses_grid_sizes(self):
        self.create_post(slug='first-image', site_slug=PRODUCT_SITE)
        self.create_post(slug='second-image', site_slug=PRODUCT_SITE)
        source_data = {
    ...
    def test_product_blog_prioritizes_article_featured_image(self):
        post = self.create_post(site_slug=PRODUCT_SITE)
        ...
        with patch('apps.blog.rendering.image_sources', return_value=source_data):
            response = self.client.get('/en/blog/english-post/', HTTP_HOST='product.localhost')
    ...
    def test_personal_and_secondary_site_lists_prioritize_only_the_first_image(self):
        source_data = {
            'original': '/media/original.webp',
            'src': '/media/rendition.webp',
            'srcset': '',
        ...
        cases = (
            ...
            (SECONDARY_SITE, 'recipes.example.com', '/en/blog/'),
        )
    ...
    def test_detail_renders_shared_blocks_and_article_metadata(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'personal_site/blog/detail.html')
        ...
        self.assertContains(response, static('personal_site/css/blog.css'))
        self.assertEqual(response.headers['Content-Language'], 'en')
    ...
    def test_embed_script_is_conditionally_loaded_in_every_detail_shell(self):
        ...
        cases = (
            ...
            (SECONDARY_SITE, 'recipes.example.com'),
            (PRODUCT_SITE, 'admin-theme.example.com'),
        )
    ...
    def test_public_detail_exposes_sharing_and_read_mode_on_all_site_shells(self):
        cases = (
            (PERSONAL_SITE, 'testserver', 'personal_site/css/blog.css'),
            (SECONDARY_SITE, 'recipes.example.com', 'blog/css/article.css'),
            (PRODUCT_SITE, 'admin-theme.example.com', 'product_site/css/blog.css'),
        )
    ...
    def test_author_archive_filters_public_posts_and_is_indexable_with_two_posts(self):
        ...
        self.create_post(slug='example-author-first', author=author)
        self.create_post(slug='example-author-second', author=author)
        ...
        self.assertContains(response, '/en/blog/example-author-first/')
        self.assertContains(response, '/en/blog/example-author-second/')
    ...
    def test_single_article_author_archive_is_noindex_and_missing_author_is_404(self):
        self.create_post(slug='example-author-only', author=self.create_author_profile(public_author_name='example-author'))
    ...
    def test_tag_archive_is_site_scoped(self):
        ...
        easy_post = self.create_post(slug='easy-django', site_slug=SECONDARY_SITE)
    ...
    def test_secondary_site_uses_its_shell_and_primary_canonical(self):
        self.create_post(site_slug=SECONDARY_SITE, canonical_site_slug=PERSONAL_SITE)

        response = self.client.get('/en/blog/english-post/', HTTP_HOST='recipes.example.com')

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'secondary_site/blog/detail.html')
    ...
    def test_article_seo_ignores_request_scheme_and_alternate_publication_host(self):
        ...
        social_image_url = f'https://recipes.example.com{static("secondary_site/img/logo.png")}'
    ...
    def test_all_three_sites_serve_blog_with_site_owned_social_images(self):
        ...
        cases = (
            (
                PERSONAL_SITE,
                'testserver',
                'personal_site/blog/detail.html',
                'personal_site/img/avatar.png',
                'personal_site/css/blog.css',
                {'@type': 'Person', 'name': 'Example Author', 'url': 'https://personal.example.com/'},
                ('image', 'personal_site/img/avatar.png'),
            ),
            (
                SECONDARY_SITE,
                'recipes.example.com',
                'secondary_site/blog/detail.html',
                'secondary_site/img/logo.png',
                'blog/css/article.css',
                {'@type': 'Organization', 'name': 'Secondary Site', 'url': 'https://recipes.example.com/'},
                ('logo', 'secondary_site/img/logo.png'),
            ),
            (
                PRODUCT_SITE,
                'admin-theme.example.com',
                'product_site/blog/detail.html',
                'product_site/img/social-preview.png',
                'product_site/css/blog.css',
                {'@type': 'Organization', 'name': 'Product Site', 'url': 'https://admin-theme.example.com/en/'},
                ('logo', 'product_site/img/logo.png'),
            ),
        )
        for index, (site_slug, host, template, social_image, stylesheet, publisher, publisher_image) in enumerate(cases):
            with self.subTest(site_slug=site_slug):
                ...
                post.author = self.create_author_profile(
                    public_author_name='example-author',
                    username=f'example-author-{index}',
                )
    ...
    def test_blog_stylesheet_falls_back_for_unknown_site(self):
        ...
        self.assertEqual(get_blog_stylesheet(PERSONAL_SITE), 'personal_site/css/blog.css')
        self.assertEqual(get_blog_stylesheet(SECONDARY_SITE), 'blog/css/article.css')
        self.assertEqual(get_blog_stylesheet(PRODUCT_SITE), 'product_site/css/blog.css')

    def test_standalone_blog_lists_describe_social_images(self):
        cases = (
            (SECONDARY_SITE, 'recipes.example.com', 'Secondary Site logo'),
            (PRODUCT_SITE, 'admin-theme.example.com', 'Product Site interface preview'),
        )
    ...
    def test_all_three_sites_advertise_their_own_english_feed_only_on_list(self):
        cases = (
            ...
            (SECONDARY_SITE, 'recipes.example.com', 'Secondary Site Blog RSS'),
            (PRODUCT_SITE, 'admin-theme.example.com', 'Product Site Blog RSS'),
        )
    ...
    def test_faq_uses_shared_markup_and_each_site_blog_stylesheet(self):
        cases = (
            ('personal.example.com', 'personal_site/css/blog.css'),
            ('secondary.localhost', 'blog/css/article.css'),
            ('product.localhost', 'product_site/css/blog.css'),
        )
        for index, (host, stylesheet) in enumerate(cases):
            slug = f'faq-shell-{index}'
            site_slug = (PERSONAL_SITE, SECONDARY_SITE, PRODUCT_SITE)[index]
            post = self.create_post(slug=slug, site_slug=site_slug)
            BlogFAQBlock.objects.create(
                parent=post,
                region='main',
