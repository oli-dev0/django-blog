...
class BlogSelectorTests(TestCase):
    ...
    def test_public_posts_are_site_scoped_and_newest_first(self):
        ...
        self.create_post(slug='secondary-site', site_slug=SECONDARY_SITE)

        self.assertEqual(list(get_public_posts(site_slug=PERSONAL_SITE)), [newer, older])
        self.assertEqual(list(get_public_posts(site_slug=SECONDARY_SITE)), [BlogPost.objects.get(slug='secondary-site')])
    ...
    def test_public_related_selector_fails_closed_for_stale_cross_site_links(self):
        ...
        BlogPostPublication.objects.create(post=valid_target, site_slug=PRODUCT_SITE)
        post = source
        ...
        self.assertEqual(
            list(get_related_public_posts(post=post, site_slug=PRODUCT_SITE)),
            [valid_target],
        )
