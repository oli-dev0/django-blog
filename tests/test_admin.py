...
class BlogAdminTests(TestCase):
    def setUp(self):
        ...
        BlogSite.objects.bulk_create(
            [BlogSite(slug=slug) for slug in (PERSONAL_SITE, SECONDARY_SITE, PRODUCT_SITE)],
            ignore_conflicts=True,
        )
        self.category = BlogCategory.objects.create(name='General', slug='general')
        self.category.websites.add(PERSONAL_SITE, SECONDARY_SITE, PRODUCT_SITE)
    ...
    def test_preview_redirects_to_default_and_uses_selected_site_shell(self):
        ...
        self.assertRegex(
            ...
            r'/static/personal_site/css/blog\.[0-9a-f]+\.css',
        )
        ...
        easy_response = self.client.get(
            f'/admin/blog/blogpost/{post.pk}/preview/?site={SECONDARY_SITE}',
            HTTP_HOST='admin.localhost',
        )

        self.assertEqual(easy_response.status_code, 200)
        self.assertTemplateUsed(easy_response, 'secondary_site/blog/detail.html')
        ...
        product_response = self.client.get(
            f'/admin/blog/blogpost/{post.pk}/preview/?site={PRODUCT_SITE}',
            HTTP_HOST='admin.localhost',
        )

        self.assertEqual(product_response.status_code, 200)
        self.assertTemplateUsed(product_response, 'product_site/blog/detail.html')
        self.assertRegex(product_response.content.decode(), r'/static/product_site/css/blog\.[0-9a-f]+\.css')
        self.assertNotRegex(product_response.content.decode(), r'/static/blog/css/article\.[0-9a-f]+\.css')
        self.assertNotContains(product_response, 'plausible.personal.example.com')
        self.assertNotContains(product_response, 'type="application/rss+xml"', html=False)
        self.assertNotContains(product_response, 'data-blog-read-mode-toolbar', html=False)
        for preview_response in (response, easy_response, product_response):
    ...
    def test_preview_default_order_is_canonical_then_assigned_then_configured(self):
        post = self.create_post(
            ...
            canonical_site_slug=PRODUCT_SITE,
        )
        BlogPostPublication.objects.create(post=post, site_slug=PERSONAL_SITE)
        self.assertEqual(resolve_preview_site_slug(post), PRODUCT_SITE)
    ...
    def test_related_form_filters_targets_by_all_projected_sites(self):
        ...
        BlogPostPublication.objects.create(post=source, site_slug=PRODUCT_SITE)
        ...
        BlogPostPublication.objects.create(post=both_sites, site_slug=PRODUCT_SITE)

        form = BlogPostRelatedForm(source_post=source)

        self.assertEqual(list(form.fields['related_post'].queryset), [both_sites])
        self.assertIn('Product Site', form.fields['related_post'].help_text)
    ...
    def test_related_form_rejects_manually_submitted_incompatible_target(self):
        ...
        BlogPostPublication.objects.create(post=target, site_slug=SECONDARY_SITE)
    ...
    def test_related_formset_uses_projected_publication_sites(self):
        ...
        formset = formset_class(
            data={
                'publication_sites': [PERSONAL_SITE, PRODUCT_SITE],
    ...
    def test_non_organizer_related_formset_ignores_forged_publication_sites(self):
        ...
        BlogPostPublication.objects.create(post=source, site_slug=PERSONAL_SITE)
        target = self.create_post(title='Product target', slug='restricted-target')
        BlogPostPublication.objects.create(post=target, site_slug=PRODUCT_SITE)
        formset_class = inlineformset_factory(
            BlogPost,
            BlogPostRelated,
            form=BlogPostRelatedForm,
        ...
        formset = formset_class(
            data={
                'publication_sites': PRODUCT_SITE,
                'related_links-TOTAL_FORMS': '1',
                'related_links-INITIAL_FORMS': '0',
                'related_links-MIN_NUM_FORMS': '0',
                'related_links-MAX_NUM_FORMS': '1000',
    ...
    def test_site_fields_use_blog_enabled_select_choices(self):
        ...
        self.assertIn(PRODUCT_SITE, dict(post_form.fields['canonical_site_slug'].choices))
        self.assertIn(PRODUCT_SITE, dict(publication_form.fields['site_slug'].choices))
        self.assertIn(PRODUCT_SITE, dict(post_form.fields['publication_sites'].choices))
        self.assertEqual(
    ...
    def test_tag_admin_requires_websites_and_generates_slug(self):
        ...
        save_response = self.client.post(
            '/dev-admin/blog/blogtag/add/',
            {'name': 'Release notes', 'available_websites': [PRODUCT_SITE], '_save': 'Save'},
            HTTP_HOST='dev-admin.localhost',
        )
