...
from apps.core.sites import SECONDARY_SITE, PERSONAL_SITE, PRODUCT_SITE, get_site_definition
...
class BlogSitemapTests(TestCase):
    def create_post(self, *, slug, canonical_site=PERSONAL_SITE):
        ...
        BlogPostPublication.objects.create(post=post, site_slug=SECONDARY_SITE)
    ...
    def test_noncanonical_site_does_not_get_detail_entry(self):
        self.create_post(slug='easy-canonical', canonical_site=SECONDARY_SITE)
    ...
    def test_product_sitemap_includes_enabled_blog(self):
        request = RequestFactory().get('/sitemap.xml', HTTP_HOST='admin-theme.example.com')

        entries = get_sitemap_entries(
            request=request,
            site=get_site_definition(PRODUCT_SITE),
            languages=['en'],
        )
    ...
    def test_sitemap_includes_indexable_author_archive(self):
        first = self.create_post(slug='example-author-first')
        second = self.create_post(slug='example-author-second')
