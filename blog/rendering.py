...
def get_blog_stylesheet(site_slug):
    return getattr(settings, 'BLOG_STYLESHEET_PATHS', {}).get(site_slug, BLOG_FALLBACK_STYLESHEET)
...
def get_site_social_image(site_slug):
    image = getattr(settings, 'BLOG_SITE_SOCIAL_IMAGES', {}).get(site_slug)
    if not image:
        return {'url': '', 'alt': ''}
    path, alt = image
...
def _publisher_schema(site_slug):
    publisher = getattr(settings, 'BLOG_SITE_PUBLISHERS', {}).get(site_slug)
    if not publisher:
        return None
