...
The Blog emits server-rendered SEO metadata for the sites enabled by the host
project's registry. This showcase does not ship a production site registry.
Use the app-owned `blog` template namespace for the neutral reference frontend,
or provide a custom shell with its own branding and assets.

The Blog is English-only. The host may mount it at `/blog/` or `/en/blog/`.
No translated article variants or Blog hreflang cluster are emitted.
...
Configure `BLOG_SITE_PUBLISHERS` with a real `Person` or `Organization`
identity per site. `BLOG_SITE_SOCIAL_IMAGES` separately maps site slugs to
static image paths and alt text. Both settings default to empty mappings, so
unconfigured publisher and social-image metadata is omitted. Never use an
interface screenshot as an organization's logo. Missing optional facts are
omitted rather than invented. Private account fields and unsupported ratings,
reviews, offers, prices, FAQ, Product, or SoftwareApplication claims are not
exposed.
...
unprefixed routes and `/en/blog/rss/` for prefixed routes), limited to the
latest 20 effectively published articles. Item URLs use each article's trusted canonical
identity.
...
- `apps.core.sites` (host-provided): trusted origins and absolute URLs.
...
- Host URL configuration: mount sitemap providers and provide robots responses.
