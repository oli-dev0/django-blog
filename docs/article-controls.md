...
- Public articles using the shared article partial render an
...
- Public articles using the shared article partial render a printer icon with the
...
- Print styling lives in `blog/static/blog/css/article.css`. Custom host
  stylesheets must preserve these rules. The button remains hidden
  when the browser does not expose `window.print`; saved Admin previews never
  render it.

## Sharing the article

- Public articles using the shared article partial render an icon-only `Share` action
...
The reference shell uses `blog/static/blog/css/article.css`, which contains
read-mode layout, responsive reflow, theme-token, forced-colors,
...
Custom host headers and navigation must match the shared shell-hide selectors
or supply equivalent read-mode rules. Check the actual host layout when
integrating these controls.

## Verification and tests

Server-rendered coverage is in:

- `tests/blog/test_views.py` for the host test shells, tagged/no-tag rows,
