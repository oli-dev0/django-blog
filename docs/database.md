...
Blog-site taxonomy tables and backfills existing categories and tags only to
each site already used by their article publication rows. Its reverse
operation is a no-op. No default site is created; unused taxonomy terms
remain unassigned until an editor configures their website availability.
