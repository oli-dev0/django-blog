...
per-website assignments. Existing terms are backfilled only to sites
...
Views choose the consuming site's list/detail shell. `apps.blog` owns selection, workflow, blocks, shared article presentation, metadata, RSS, and sitemap rules; custom host shells own their headers, footers, and branding. The bundled reference frontend provides a neutral alternative.
...
Hosts configure real `Person` or `Organization` publishers through
`BLOG_SITE_PUBLISHERS`, separately from `BLOG_SITE_SOCIAL_IMAGES`. Both default
to empty mappings to avoid invented identities or references to absent assets.
